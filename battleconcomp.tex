...
\gamerule{Base Cards}{
  \label{basecards}
  \subrules{
    \item Base cards list their stats on the left side, started with "+"s (pluses). Every Attack Pair must contain at least one base.
    \subsubrules{\item Some effects or character allow or require the playing of only a base. Such an Attack Pair must still include a base.}
    \item Each character kit will contain at least one character-specific base. These bases work exactly the same as the generic bases.
  }
}
